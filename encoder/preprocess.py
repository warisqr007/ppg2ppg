...
def preprocess_l2arctic_accent_embedding(datasets_root: Path, out_dir: Path, skip_existing=False):
    dataset_name = "L2ARCTIC"
    dataset_root, logger = _init_preprocess_dataset(dataset_name, datasets_root, out_dir)
...
def preprocess_saa(datasets_root: Path, out_dir: Path, skip_existing=False):
    # Initialize the preprocessing
    dataset_name = "SpeechAccentArchive"
    dataset_root, logger = _init_preprocess_dataset(dataset_name, datasets_root, out_dir)
    if not dataset_root:
        return
    ...
    # Get the speaker directories
    # Preprocess all speakers
    speaker_dirs = list(dataset_root.joinpath("SpeechAccentArchive").glob("*"))
    _preprocess_speaker_dirs(speaker_dirs, dataset_name, datasets_root, out_dir, "wav",
                             skip_existing, logger)