...
from synthesizer_like_translator.utils.text import text_to_sequence
...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with tf.device("/cpu:0"):
			...
			self._placeholders = [
				tf.compat.v1.placeholder(tf.float32, shape=(None, None, hparams.num_ppgs), name="inputs"),
			...
			queue = tf.compat.v1.FIFOQueue(8, [tf.float32, tf.int32, tf.float32, tf.float32, 
			...
			eval_queue = tf.compat.v1.FIFOQueue(1, [tf.float32, tf.int32, tf.float32, tf.float32,  
