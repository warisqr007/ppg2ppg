from synthesizer_like_translator.utils.symbols import symbols
from synthesizer_like_translator.utils.text import sequence_to_text
from synthesizer_like_translator.hparams import hparams_debug_string
from synthesizer_like_translator.feeder import Feeder
from synthesizer_like_translator.models import create_model
from synthesizer_like_translator.utils import ValueWindow, plot
from synthesizer_like_translator import infolog, audio
...
def add_train_stats(model, hparams):
    with tf.compat.v1.variable_scope("stats") as scope:
        for i in range(hparams.tacotron_num_gpus):
            print(model.tower_mel_outputs[i])
            print(model.tower_mel_targets[i])
...
def get_variables_to_restore(variables, var_to_shape_map):
    variables_to_restore = []
    for v in variables:
        # one can do include or exclude operations here.
        if list(v.shape) == var_to_shape_map[v.name.split(':')[0]] and v.name != 'global_step:0':
...
def train(log_dir, args, hparams):
    ...
    if args.warm_start_path is not None:
        ...
        checkpoint_state = tf.compat.v1.train.get_checkpoint_state(args.warm_start_path)
        var_to_shape_map = get_variable_in_checkpoint_file(checkpoint_state.model_checkpoint_path)
        # var_keep_dic = tf.contrib.framework.get_variables_to_restore()
        # var_keep_dic = {x.name: x for x in var_keep_dic}
        variables_to_restore = get_variables_to_restore(variables, var_to_shape_map)
        saver = tf.compat.v1.train.Saver(variables_to_restore, max_to_keep=5)
    else:
        ...
        try:
            ...
            if args.restore:
                ...
                try:
                    if args.warm_start_path is not None:
                        checkpoint_state = tf.compat.v1.train.get_checkpoint_state(args.warm_start_path)
                        log("Loading warm start checkpoint {}".format(checkpoint_state.model_checkpoint_path),
                            slack=True)
                    else:
                        checkpoint_state = tf.compat.v1.train.get_checkpoint_state(save_dir)
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                # print(step)
                # print(time_window.average)
                # print(loss)
                # print(loss_window.average)
