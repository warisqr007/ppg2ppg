...
from translator.module import *
from translator.utils import get_positional_table, get_sinusoid_encoding_table
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = MelDecoder(hp.hidden_size_decoder)
        #self.linear = Linear(hp.hidden_size+hp.embedding_size, hp.hidden_size)

    def forward(self, characters, embed, mel_input, pos_text, pos_mel):
        memory, c_mask, attns_enc = self.encoder.forward(characters, pos=pos_text)
        memory = t.cat((memory, embed.unsqueeze(1).repeat(1, memory.size(1), 1)), dim=2)
