import translator.hyperparams as hp
import pandas as pd
...
class TranslatorDatasets(Dataset):
    """L2-Arctic Translator dataset."""
    ...
    def __init__(self, data_dir):
        ...
        """
        metadata_fpath = Path(os.path.join(data_dir, "train.txt"))
        self.embed_dir = os.path.join(data_dir, "embeds")
        self.ppg_dir = os.path.join(data_dir, "ppgs")
        self.mel_dir = os.path.join(data_dir, "mels")
        
        # Gather the input wave filepath and the target output embed filepath
        with metadata_fpath.open("r") as metadata_file:
            self.metadata = [line.split("|") for line in metadata_file]
    ...
    def __getitem__(self, idx):
        ...
        text = np.load(os.path.join(self.ppg_dir, m[2]))
        embedding = np.load(os.path.join(self.embed_dir, m[3]))
        mel = np.load(os.path.join(self.mel_dir, m[1]))
        ...
        sample = {'text': text, 'mel': mel, 'embed': embedding, 'text_length':text_length, 'mel_input':mel_input, 'pos_mel':pos_mel, 'pos_text':pos_text}
...
def collate_fn_transformer(batch):
    ...
    if isinstance(batch[0], collections.Mapping):
        ...
        embed = [i for i, _ in sorted(zip(embed, text_length), key=lambda x: x[1], reverse=True)]
        ...
        return t.FloatTensor(np.array(text)), t.FloatTensor(np.array(mel)), t.FloatTensor(np.array(embed)), t.FloatTensor(np.array(mel_input)), t.LongTensor(np.array(pos_text)), t.LongTensor(np.array(pos_mel)), t.LongTensor(np.array(text_length))
...
def get_dataset():
    return TranslatorDatasets(hp.data_path)  #LJDatasets(os.path.join(hp.data_path,'metadata.csv'), os.path.join(hp.data_path,'wavs'))
